let flock; // L'objet qui gère tous les Boids

function setup() {
  createCanvas(800, 600); // Crée un canvas de 800x600 pixels
  // La couleur en p5.js (HSL, Saturation, Luminosité)
  colorMode(HSL, 360, 100, 100); 

  flock = new Flock();
  // Ajoute 100 Boids initiaux
  for (let i = 0; i < 100; i++) {
...
function draw() {
  background(0, 0, 0, 0.5); // Arrière-plan noir légèrement transparent (pour effet de traînée)
  flock.run();
}

// Fonction pour ajouter un nouveau Boid avec un clic de souris
function mouseDragged() {
  flock.addBoid(new Boid(mouseX, mouseY));
}

// Classe pour gérer l'ensemble des Boids
class Flock {
  constructor() {
    this.boids = []; // Tableau qui stocke tous les objets Boid
  }

  run() {
    for (let boid of this.boids) {
      // Chaque Boid exécute sa logique en se basant sur tous les autres Boids
      boid.run(this.boids);
    }
  }

...
export default function SimulationControlPanel() {
    ...
    return (
        <div style={{
            ...
            {/* Transport Control Bar (1:1 JavaFX 8 Buttons + Interactive Timeline Scrubber) */}
            <div style={{
                background: cardBg,
                border: `1px solid ${borderCol}`,
                borderRadius: 10,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <button
                            ...
                            title="⏮ Début (Tick 0)"
                            style={{ background: isDark ? '#334155' : '#e2e8f0', border: 'none', color: textMain, padding: '8px 12px', borderRadius: 6, cursor: 'pointer' }}
                        >
                            <SkipBack size={15} />
                        </button>
                        ...
                        {running ? (
                            <button
                                onClick={pause}
                                title="⏸ Pause"
                                style={{ background: '#ef4444', border: 'none', color: '#fff', padding: '8px 18px', borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 800, fontSize: 13 }}
                            >
                                <Pause size={16} /> PAUSE
                            </button>
                        ) : (
                            <button
                                ...
                                style={{ background: '#10b981', border: 'none', color: '#fff', padding: '8px 18px', borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 800, fontSize: 13 }}
                            >
                                <Play size={16} /> LECTURE
                            </button>
                        )}

                        <button
                            onClick={stepForward}
                            title="▶ Pas avant (+1 tick)"
                            style={{ background: isDark ? '#334155' : '#e2e8f0', border: 'none', color: textMain, padding: '8px 12px', borderRadius: 6, cursor: 'pointer' }}
                        >
                            <SkipForward size={13} />
                        </button>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: textMuted }}>Vitesse :</span>
                        <input
                            type="range"
                            min="0.1"
                            max="50"
                            ...
                            style={{ width: 110, accentColor: '#38bdf8' }}
                        />
                        <span style={{ fontSize: 12, fontWeight: 800, color: '#38bdf8', minWidth: 40 }}>
                            {speed.toFixed(1)}x
                        </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: textMuted }}>Pas dt :</span>
                        <select
                            value={stepSeconds}
                            onChange={(e) => setStepSeconds(parseFloat(e.target.value))}
                            style={{ background: inputBg, color: textMain, border: `1px solid ${borderCol}`, borderRadius: 6, padding: '5px 8px', fontSize: 12 }}
                        >
                            <option value="0.0166">16.6 ms (60 Hz)</option>
                            <option value="0.0333">33.3 ms (30 Hz)</option>
                            <option value="0.05">50 ms (20 Hz - Standard)</option>
                            <option value="0.1">100 ms (10 Hz)</option>
                            <option value="0.5">500 ms (2 Hz)</option>
                            <option value="1.0">1.0 s (1 Hz Macro)</option>
                        </select>
                    </div>
                </div>

                {/* Interactive Timeline Scrubber Slider (1:1 with JavaFX Timeline Slider) */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, borderTop: `1px solid ${borderCol}`, paddingTop: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: textMuted, minWidth: 70 }}>
                        Chronologie :
                    </span>
                    <input
                        type="range"
                        min="0"
                        max={Math.max(highestRecordedTick, 100)}
                        value={ticks}
                        onChange={(e) => seekToTick(parseInt(e.target.value))}
                        style={{ flex: 1, accentColor: '#0284c7' }}
                    />
                    <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#38bdf8', minWidth: 100, textAlign: 'right' }}>
                        Tick {ticks} / {highestRecordedTick}
                    </span>
                </div>
            </div>
